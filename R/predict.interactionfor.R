...
##' Prediction with new data and a saved interaction forest from \code{\link{interactionfor}}.
##' 
##' Note that his package is a fork of the R package 'ranger' that implements random forests using an
...
##'
##' @title Interaction Forest prediction
##' @param object \code{interactionfor} object.
...
##' @return Object of class \code{interaction.prediction} with elements
##'   \tabular{ll}{
...
##'   }
##' @seealso \code{\link{interactionfor}}
##' @author Marvin N. Wright, Roman Hornung
##' @export
predict.interactionfor <- function(object, data = NULL, predict.all = FALSE,
  ...
  if (is.null(forest)) {
    stop("Error: No saved forest in interactionfor object. Please set write.forest to TRUE when calling divfor.")
  }
  ...
  if (type == "quantiles") {
    ...
    if (is.null(object$random.node.values)) {
      stop("Error: Set quantreg=TRUE in interactionfor(...) for quantile prediction.")
    }
    
    if (is.null(data)) {
      ## OOB prediction
      if (is.null(object$random.node.values.oob)) {
        stop("Error: Set keep.inbag=TRUE in interactionfor(...) for out-of-bag quantile prediction or provide new data in predict(...).")
      }
    ...
    result <- list(num.samples = nrow(node.values),
                   ...
                   num.trees = num.trees)
    class(result) <- "interactionfor.prediction"
...
predict.interactionfor.forest <- function(object, data, predict.all = FALSE,
  ...
  if (!inherits(object, "interactionfor.forest")) {
  ...
  ## Check for old interactionfor version
  if (length(forest$child.nodeIDs) != forest$num.trees || length(forest$child.nodeIDs[[1]]) != 2) {
    stop("Error: Invalid forest object. Is the forest grown in interactionfor version <0.3.9? Try to predict with the same version the forest was grown.")
  }
  ...
  if (type == "response" || type == "se") {
    ...
    stop("Error: Apply predict() to the interactionfor object instead of the $forest object to predict quantiles.")
  } else {
  ...
  if (type == "se" && is.null(inbag.counts)) {
    stop("Error: No saved inbag counts in interactionfor object. Please set keep.inbag=TRUE when calling interactionfor.")
  }
